...
def detect_rumble_regions(freqs: np.ndarray, times: np.ndarray, magnitude: np.ndarray) -> list[DetectionSegment]:
    power = magnitude ** 2
    # Broaden masks to capture any elephant vocalization
    # Rumbles: 8-180 Hz
    rumble_mask = band_mask(freqs, RUMBLE_LO, HARMONIC_HI)
    # Mid/High vocalizations (Trumpets, Roars, Squeaks): 180-8000 Hz
    trumpet_mask = band_mask(freqs, HARMONIC_HI, OUTPUT_HI)
    # Background noise reference is generally higher or completely broad
    # For a general VAD, we penalize very steady noise or extremely high frequencies not in biological range
    ultra_high_mask = band_mask(freqs, OUTPUT_HI, OUTPUT_HI + 4000.0)

    rumble_energy = np.log1p(power[rumble_mask, :].sum(axis=0))
    trumpet_energy = np.log1p(power[trumpet_mask, :].sum(axis=0))
    ultra_high_energy = np.log1p(power[ultra_high_mask, :].sum(axis=0))

    # Calculate robust z-scores to find regions that stand out from background
    z_rumble = robust_zscore(smooth_frames(rumble_energy, 2.5))
    z_trumpet = robust_zscore(smooth_frames(trumpet_energy, 2.5))
    z_ultra = robust_zscore(smooth_frames(ultra_high_energy, 3.0))

    # Broad score favors ANY strong vocalization (rumble OR trumpet/roar)
    broad_score = np.maximum(z_rumble, z_trumpet) - 0.2 * z_ultra
    broad_score = smooth_frames(broad_score, sigma_frames=4.0)

    # Use a slightly stricter threshold to avoid false positives on background noise bursts
    threshold = np.percentile(broad_score, 60)
    support = broad_score > threshold

    frame_step = times[1] - times[0] if len(times) > 1 else HOP / 44100.0

    # Call-level envelope: responsive envelope that spikes during ANY vocalization event
    call_envelope = smooth_frames(
        0.5 * rumble_energy + 0.5 * trumpet_energy - 0.2 * ultra_high_energy,
        sigma_frames=1.8,
    )
...
def soft_frequency_mask(freqs: np.ndarray) -> np.ndarray:
    ...
    for index, freq in enumerate(freqs):
        ...
        elif freq <= OUTPUT_HI - 500.0:
            mask[index] = 1.0
        elif freq <= OUTPUT_HI:
            mask[index] = max(0.0, 1.0 - (freq - (OUTPUT_HI - 500.0)) / 500.0)
        else:
            mask[index] = 0.0
...
def clean_audio(audio_path: str | Path, output_path: str | Path) -> CleaningSummary:
    ...
    nyquist = sr / 2.0
    if nyquist > RUMBLE_LO:
        b, a = butter(4, RUMBLE_LO / nyquist, btype="high")
        cleaned_wave = filtfilt(b, a, cleaned_wave)
    if nyquist > OUTPUT_HI + 5:
        b, a = butter(4, OUTPUT_HI / nyquist, btype="low")
        cleaned_wave = filtfilt(b, a, cleaned_wave)
    elif nyquist > 200:
        b, a = butter(4, (nyquist * 0.95) / nyquist, btype="low")
        cleaned_wave = filtfilt(b, a, cleaned_wave)
